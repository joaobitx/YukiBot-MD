...
export default {
  ...
  run: async ({ msg, sock, usedPrefix, command }) => {
    try {
      ...
      (global.db.data.chats[msg.chat]?.users?.[msg.sender] && (global.db.data.chats[msg.chat].users[msg.sender].robVictims ??= {}));      
      let userData = global.db.data.chats[msg.chat]?.users?.[msg.sender];
      ...
      if (target === msg.sender) {
        const robberName = (global.db.data.users[msg.sender])?.name?.trim() || msg.sender.split('@')[0];
        return msg.reply(`ꕥ No puedes robarte a ti mismo, *${robberName}*.`);
      }
      const targetUser = global.db.data.chats[msg.chat]?.users?.[target];
      const lastCmd = targetUser?.lastCmd || 0;
      const tiempoInactivo = now - lastCmd;
      if (tiempoInactivo < 3600000) {
        const targetName = (global.db.data.users[target])?.name?.trim() || target.split('@')[0];
        return msg.reply(`ꕥ *${targetName}* estuvo activo recientemente. Solo puedes robarle waifus si ha estado inactivo más de 1 hora.`);
      }
      (global.db.data.chats[msg.chat]?.users?.[target] && (global.db.data.chats[msg.chat].users[target].favorite ??= ''));
      let victim = global.db.data.chats[msg.chat]?.users?.[target];
      ...
      global.db.data.chats[msg.chat].users[msg.sender].lastrobwaifu = userData.lastrobwaifu;      
      if (!success) {
      ...
      global.db.data.chats[msg.chat].users[msg.sender].robVictims = userData.robVictims;
      ...
      (global.db.data.characters[charKey] ||= {}, global.db.data.characters[charKey].name ??= '', global.db.data.characters[charKey]);
      let characterData = global.db.data.characters[charKey];
      if (!characterData) {
      ...
      global.db.data.characters[charKey] = characterData;
      victim.characters = victim.characters.filter(id => id !== stolenId);
      global.db.data.chats[msg.chat].users[target].characters = victim.characters;
      if (!userData.characters.includes(stolenId)) {
        userData.characters.push(stolenId);
        global.db.data.chats[msg.chat].users[msg.sender].characters = userData.characters;
      }
      if (victim.favorite === stolenId) {
        global.db.data.chats[msg.chat].users[target].favorite = '';
        global.db.data.users[target].favorite = '';
      }      
